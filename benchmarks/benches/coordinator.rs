...
use coordinator::proto::{
    BarrierRequest, DatasetInfo, HeartbeatRequest, ResourceUsage, ShardRequest, WorkerInfo,
    WorkerStatus,
};
use coordinator::{CoordinatorClient, CoordinatorService, CoordinatorServiceServer};
...
fn bench_heartbeat_processing(c: &mut Criterion) {
    ...
    let worker_ids: Vec<String> = (0..10).map(|i| format!("worker-{}", i)).collect();
                        ...
                        rt.block_on(async {
                            ...
                            for i in 0..workers {
                                ...
                                let handle = tokio::spawn(async move {
                                    let mut client =
                                        CoordinatorClient::connect(addr).await.unwrap();
                                    client
